...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        pbar = tqdm(list(range(self.num_timesteps))[::-1], disable=disable_tqdm)
        for idx in pbar:
    ...
    def p_mean_variance(self, model, x, t, u=None):
        model_output = model(x, self._scale_timesteps(t), u=u)
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, model, use_timesteps, **kwargs):
        ...
        base_diffusion = GaussianDiffusion(model, **kwargs)  # pylint: disable=missing-kwoa
...
class DDIM(SpacedDiffusion):
    def p_sample(self, model, x, t, eta=1.0, control_eta=1.0, u=None):
        out = self.p_mean_variance(model, x, t, u=u)
        ...
        control = (torch.sqrt(1 - alpha_bar_prev - sigma_control ** 2)
                   - torch.sqrt(1 - alpha_bar_prev - sigma ** 2)) * eps
